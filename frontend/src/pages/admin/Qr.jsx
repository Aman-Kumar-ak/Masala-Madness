    ...
    <div className="flex flex-col min-h-screen bg-gray-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100">
      ...
      {isSecretCodeAccessGranted && isLoggedIn && (
        <div className="flex-1 flex flex-col">
          <div className="fixed top-0 left-0 right-0 z-10 p-4 bg-gray-100 dark:bg-zinc-900 shadow-md">
            <BackButton />
            <h1 className="text-3xl font-bold text-center mt-4 mb-6">QR Code Generator</h1>
          </div>

          <div className="flex-1 overflow-y-auto pt-24 p-4 mt-10 md:p-8 flex flex-col items-center">
            {/* Session Expiry Timer */}
            {isSecretCodeAccessGranted && sessionTimeLeft && (
              <div className="text-center mb-4">
                <p className="text-sm text-gray-700 dark:text-gray-300">Session expires in: {sessionTimeLeft}</p>
              </div>
            )}

            {/* Main content with conditional blur */}
            <div className={!isSecretCodeAccessGranted ? 'filter blur-md pointer-events-none' : ''}>
